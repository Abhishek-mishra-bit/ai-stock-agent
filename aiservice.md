...
`;
    ...
    try {
      ...
      } else if (response.output?.[1]?.content?.[0]?.text) {
        analysisContent = response.output[1].content[0].text;
      } else if (response.choices?.[0]?.text) {
        analysisContent = response.choices[0].text;
      }

      if (!analysisContent || analysisContent.trim() === "") {
        console.warn("Warning: Empty analysis received. Response:", JSON.stringify(response).substring(0, 500));
        analysisContent = `

Technical Analysis for ${stockData.companyName}:

- Current Price: ₹${currentPrice}
- Signal: ${technicalIndicators.signal}
- Confidence: ${technicalIndicators.confidence}%
- Key Factors: ${technicalIndicators.reasons.join(", ")}

Note: Full AI analysis could not be generated. Check your API key and model access.
`;
}
...
`;
    ...
    try {
      ...
      const sentimentContent =
        ...
        "Unable to analyze sentiment at this time.";
